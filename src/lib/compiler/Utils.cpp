...
const map<string, string> Utils::BASIC_ESCAPE{
    {"\"", "&quot;"}, // " - double-quote
    {"&", "&amp;"},   // & - ampersand
    {"<", "&lt;"},    // < - less-than
    {">", "&gt;"},    // > - greater-than
};
const map<string, string> Utils::ISO8859_1_ESCAPE{
    {"\u00A0", "&nbsp;"},   // non-breaking space
...
const map<string, string> Utils::HTML40_EXTENDED_ESCAPE{
    // <!-- Latin Extended-B -->
...
void Utils::escapeHtml4(string &str)
{
    string buf;
    map<string, string> html4Escapes;

    html4Escapes.insert(BASIC_ESCAPE.begin(), BASIC_ESCAPE.end());
    html4Escapes.insert(ISO8859_1_ESCAPE.begin(), ISO8859_1_ESCAPE.end());
    html4Escapes.insert(HTML40_EXTENDED_ESCAPE.begin(), HTML40_EXTENDED_ESCAPE.end());
    // html4Escapes.merge(BASIC_ESCAPE);
    // html4Escapes.merge(ISO8859_1_ESCAPE);
    // html4Escapes.merge(HTML40_EXTENDED_ESCAPE);
    buf.reserve(str.size() * 1.2);
    for (auto item : html4Escapes)
    {
        ...
        while ((pos = str.find(item.first, searchStart)) != string::npos)
        {
            ...
            searchStart = pos + searchLength;
        }
        buf.append(str.substr(searchStart));
        str.swap(buf);
        buf.clear();
    }
    str.shrink_to_fit();
